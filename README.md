...
カメラのみ）。原点を通る 2 本は軸の色（X=赤 / Z=青）なので向きの目印に
なります。表示の ON/OFF と間隔（0.25〜10 m、既定 1 m）は Inspector > ギズモ
から変えられます。グリッドは作業目安で、物理の床（`kGroundSize`、既定 20 m）
より広いことに注意してください - 床の外に置いた物はシミュレートで落下します。
