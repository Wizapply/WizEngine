...
- **エディタモード / シミュレートモード**: 物理を止めて配置・設計する
  ...
  13 種類のジョイント（固定・ちょうつがい・ボール・直動・距離・自在継手・
  円筒・平面・点‐線・点‐面・歯車・ねじ・ばね。可動範囲・モータ・ばね・
...
| 物性 | 選択オブジェクトの摩擦・反発・転がり・粘着（空欄 = シーン設定）、衝突レイヤと当てないレイヤ、重力を受けるか、初速・初角速度 |
| ジョイント | 選択中だけ表示。種類と軸、可動範囲・モータ・ばね・破断・歯車比を選び、A（現在の選択）と B（既定は地面）を繋ぐ。一覧は**選択が関わるジョイントのみ**で、シミュレート中は反力を表示。行をクリックで値をフォームへ、✎ で書き戻し |
...
射影 / 陰解法 / 台形則）、接触ソルバ（BB / APGD / PSOR / Jacobi / MINRES）、
2 材質の合成方式（小さい方 / 平均 / 大きい方）も選べます（積分器とソルバは
Core バックエンドのみ。Multicore は固定）。
